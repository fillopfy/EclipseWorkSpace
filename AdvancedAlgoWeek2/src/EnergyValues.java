...
class EnergyValues {
    ...
    static Position SelectPivotElement(double a[][], boolean used_raws[], boolean used_columns[]) {
        ...
        int n=used_raws.length;
        
        while (used_raws[pivot_element.raw] && pivot_element.raw<n)
            ++pivot_element.raw;
        
        while (used_columns[pivot_element.column] && pivot_element.column<n)
            ++pivot_element.column;
        
        while(a[pivot_element.raw][pivot_element.column]==0 && pivot_element.raw<n && pivot_element.column<n) {
        	++pivot_element.raw;
        	++pivot_element.column;
        }
    ...
    static void ProcessPivotElement(double a[][], double b[], Position pivot_element) {
    	...
    	double element=a[pivot_element.raw][pivot_element.column];
    	
    	for(int c=0;c<a.length;c++) {
			a[pivot_element.raw][c]/=element;
		}
    	
		b[pivot_element.raw]/=element;
		
		int col=pivot_element.column;
		
    	for(int i=pivot_element.raw+1;i<a.length;i++) {
    		double el=a[i][col];
    		for(int c=pivot_element.column;c<a.length;c++) {
